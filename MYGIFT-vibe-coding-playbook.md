...
**Backend first:** In WP, build the native mygift-core content managers (no ACF): "Global Settings" (free-shipping threshold, gift-wrap price, footer, socials, contact) and "Homepage Builder" (announcement bar + ordered/toggleable blocks: hero_slider (desktop image, mobile image, heading, sub, cta_label, cta_link)[], category_tiles[], featured_tabs (title + source category), gift_banner, occasion_chips[], from_abroad_block, trust_row[]). These expose REST under /wp-json/mygift/v1/* . Document shapes in the `headless-wp-woo` skill §4a so queries never drift.
