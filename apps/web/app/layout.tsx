...
import { AuthNudge } from '@/components/account/AuthNudge'
...
export default async function RootLayout({
  ...
  const [globalOpts, wooNav, homeContent] = await Promise.all([
    fetchGlobalOptions(),
    WOO_REST_ENABLED ? fetchWooNavCategories() : Promise.resolve(null),
    fetchHomeContent(),
  ])
  const opts = globalOpts ?? DEFAULT_GLOBAL
  const announcementBar = homeContent?.announcementBar ?? DEFAULT_ANNOUNCEMENT
